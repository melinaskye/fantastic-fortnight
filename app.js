...
app.get('/pokemon/type/:name', (req, res) => {
    ...
    for (let i = 0; i < pokemon.length; i++) {
        ...
        if(pokemon[i].type.findIndex( item => name.includes(item.toLowerCase()) ) !== -1) {
            found.push(pokemon[i])
        }
    }
...
//     res.sendFile(__dirname + '/static/img/chrome.png')
// })
...
// Variables don't show in address bar
// app.post('/', (req, res) => {
